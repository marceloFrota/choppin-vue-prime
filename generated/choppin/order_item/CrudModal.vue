<template>
    <div class="card">
        ...
        <DataTable_order_item :data="order_item_data" @remove="handleRemove" @edit="edit"></DataTable_order_item>
        <Toast />

        <Dialog v-model:visible="dialog" :style="{ width: '450px' }" :header="modalTitle" :modal="true">
            <div class="flex align-items-center justify-content-center">
                <Form_order_item :formAction="formAction" :record="order_item" @saved="onSaved"></Form_order_item>
            </div>
        ...
        <Dialog v-model:visible="deleteDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
            <div class="flex align-items-center justify-content-center">
                ...
                <span v-if="order_item"
                    >Tem certeza que quer excluir <b>{{ order_item.name }}</b
                    >?</span
                >
            </div>
            <template #footer>
                <Button label="Não" icon="pi pi-times" class="p-button-text" @click="deleteDialog = false" />
                <Button label="Sim" icon="pi pi-check" class="p-button-text" @click="remove(order_item.id)" />
            </template>
        </Dialog>
    </div>
</template>
...
import DataTable_order_item from '../../components/order_item/DataTable.vue';
import Form_order_item from '../../components/order_item/Form.vue';
...
export default {
    ...
    components: { DataTable_order_item, Form_order_item },
    created() {
        const store = useAppStore();
        store.get_order_item();
    },
    computed: {
        order_item_data() {
            const store = useAppStore();
    ...
    methods: {
        ...
        async remove(id) {
            let url = `${BASE_API_URL}/order_item/${id}`;
