    ...
    <div class="card">
        ...
        <Dialog v-model:visible="deleteDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
            <div class="flex align-items-center justify-content-center">
                ...
                <span v-if="product"
                    >Tem certeza que quer excluir <b>{{ product.name }}</b
                    >?</span
                >
            </div>
            <template #footer>
                <Button label="Não" icon="pi pi-times" class="p-button-text" @click="deleteDialog = false" />
                <Button label="Sim" icon="pi pi-check" class="p-button-text" @click="remove(product.id)" />
            </template>
        </Dialog>
    </div>
</template>
...
export default {
    ...
    components: { DataTable_product, Form_product },
    created() {
